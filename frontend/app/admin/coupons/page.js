      ...
      <div className="mt-6">
        {loading ? (
          ...
          <div className="flex flex-col gap-4">
            {coupons.map((c) => (
              <div key={c._id} className="bg-white p-4 rounded shadow flex flex-col lg:flex-row lg:items-start gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mt-2 leading-relaxed">Active: {c.isActive ? 'Yes' : 'No'} • Uses: {c.usesCount || 0} • Valid: {new Date(c.startsAt).toLocaleDateString()} → {new Date(c.endsAt).toLocaleDateString()}</p>
                </div>
                ...
                <div className="w-full lg:w-auto flex flex-col gap-2 lg:items-end">
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded text-sm whitespace-nowrap ${c.isActive ? 'bg-red-500 text-white' : 'bg-green-600 text-white'}`}
                    >
