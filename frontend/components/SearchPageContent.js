...
//             // Choose base price: discountedPrice if available, otherwise normal price
//             const basePrice = hasDiscount ? discountedPrice : room.price;

//             // If hourly booking selected, calculate proportional hourly price
...
//                   <span className="text-sm text-gray-500">({hours} hour{hours > 1 ? "s" : ""})</span>
//                 </p>
//               );
//             }

//             // Otherwise fallback to normal per-night pricing
//             if (hasDiscount) {
//               return (
...
function RoomCard({ room, city, checkInDate, checkOutDate, searchParams, onBookNow }) {
  ...
  const handleBookNow = (e) => {
    ...
    // 🕒 For hourly booking: open modal if missing info
    if (bookingType === "hourly") {
      // Show Booking Modal if no check-in date or duration selected
      if (!checkInDate || hours === 0 || !adults || !children) {
        onBookNow(room);
        return;
      }
    ...
    >
      ...
      <div className="flex-1 p-5 flex flex-col justify-between">
        <div>
          <h2 className="text-xl font-semibold text-gray-800">
            <a
              href={`/rooms/${slugify(`${room.title}-in-${room.location?.city || ''}`, {
                lower: true,
              })}/${room._id}${queryString}`}
              className="hover:text-red-500 transition-colors duration-200"
            >
              {room.title}
            </a>
