...
export default function SearchPageContent({
  ...
  const filteredRooms = useMemo(() => {
    return rooms.filter((room) => {
      ...
      // ✅ Nearby area filter (locality + nearestLocalities)
      const nearbyText = (
        (room.location?.locality || "") +
        " " +
        (room.location?.nearestLocalities || []).join(" ")
      ).toLowerCase();

      const matchesNearby =
        !nearbyQuery.trim() ||
        nearbyText.includes(nearbyQuery.toLowerCase());

      return matchesProperty && matchesNearby;
    });
  }, [rooms, propertyQuery, nearbyQuery]);
      ...
      <div className="md:col-span-1 space-y-6 lg:pt-[54px] md:pt-[54px]">
        ...
        <div>
          ...
          <input
            ...
            className="w-full border rounded-lg px-4 py-2 shadow-sm focus:ring-2 focus:ring-red-500"
          />
        </div>
